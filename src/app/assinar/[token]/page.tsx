...
import { textoTermoDivulgacao, textoTermoImagem } from "@/lib/termos";
...
export default function AssinarPage() {
  ...
  const [info, setInfo] = useState<{ clinica: string; primeiro_nome: string } | null>(null);
  ...
  const nomeTermo = nome.trim() || "[seu nome completo]";
  const clinica = info?.clinica ?? "";
  const pronto = nome.trim().split(/\s+/).length >= 2 && aceitaImagem && divulgacao !== null && !!assinatura;

  async function concluir() {
    if (!pronto || divulgacao === null) return;
    setErro("");
    setEnviando(true);
    const { data, error } = await supabase.rpc("assinar_termos", {
      p_token: token,
      p_nome: nome.trim(),
      p_aceita_divulgacao: divulgacao,
      p_texto_imagem: textoTermoImagem(nome.trim(), clinica),
      p_texto_divulgacao: textoTermoDivulgacao(nome.trim(), clinica, divulgacao),
  ...
  if (concluido !== null) {
    return (
      <main className="mx-auto flex min-h-dvh max-w-md flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-tinta/70">Obrigada, {info.primeiro_nome}. Seu consentimento foi registrado na {clinica}.</p>
  ...
  return (
    <main className="mx-auto max-w-md px-5 pb-12 pt-6">
      <header className="mb-6 text-center">
        ...
        <p className="text-tinta/60">Leia e assine os termos abaixo.</p>
      </header>
      ...
      <section className="mb-5 rounded-2xl bg-white p-4 shadow-sm">
        ...
        <label className="flex items-center gap-3">
          <input type="checkbox" className="h-6 w-6 accent-salvia" checked={aceitaImagem}
            onChange={(e) => setAceitaImagem(e.target.checked)} />
      ...
      {/* Assinatura */}
      <section className="mb-5 rounded-2xl bg-white p-4 shadow-sm">
        <h2 className="mb-2 font-semibold">3. Sua assinatura</h2>
